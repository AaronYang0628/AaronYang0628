...
if ! command -v pv &> /dev/null; then
    curl -O  -O https://ivarch.com/programs/sources/pv-1.9.31.tar.gz \
...
NO_WAIT=true
p "当前脚本将展示Gatekeeper在 *约束管理*, *资源控制* 方面的能力：如 对资源的创建和修改行为严格限制与实时监控，阻止不符合政策的资源操作，保障集群资源的规范性和一致性等"
...
p "这段代码定义了一个名为 violation 的规则(方法)，用于检查 Kubernetes 资源 是否缺少指定的标签。如果 k8s资源 缺少任何一个必需的标签，该规则就会触发违规，并生成相应的错误消息"
